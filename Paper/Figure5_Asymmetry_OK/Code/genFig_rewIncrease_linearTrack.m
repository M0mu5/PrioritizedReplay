...
params.rewMag           = [1 4]; % reward magnitude (rows: locations; columns: values)
params.rewSTD           = [0.1 0.1]; % reward Gaussian noise (rows: locations; columns: values)
...
load('..\..\Figure3_FvsR_balance\Code\genFig_FvsR_linearTrack.mat','preplayF','replayF','preplayR','replayR')
...
legend({'Unchanged','4x reward'},'Location','NortheastOutside');
set(f1(1).Parent,'XTickLabel',{'Forward correlated','Reverse correlated'});
ylim([-200 200]);
grid on
...
% Equal reward
load('..\..\Figure3_FvsR_balance\Code\genFig_FvsR_linearTrack.mat','forwardCount','reverseCount')
...
% Reward shift
diffRew_F1 = preplayF_baseline+replayF_baseline;
diffRew_F2 = preplayF_rewShift+replayF_rewShift;
diffRew_R1 = preplayR_baseline+replayR_baseline;
diffRew_R2 = preplayR_rewShift+replayR_rewShift;
% Calculate percent differences
F_1x1x = 100 * ((nanmean(eqRew_F2)-nanmean(eqRew_F1)) / nanmean(eqRew_F1));
F_1x4x = 100 * ((nanmean(diffRew_F2)-nanmean(diffRew_F1)) / nanmean(diffRew_F1));
R_1x1x = 100 * ((nanmean(eqRew_R2)-nanmean(eqRew_R1)) / nanmean(eqRew_R1));
R_1x4x = 100 * ((nanmean(diffRew_R2)-nanmean(diffRew_R1)) / nanmean(diffRew_R1));
...
%% EXPORT FIGURE
if saveBool
    save genFig_rewIncrease_linearTrack.mat
